...
export default function VendorInviteQR({ 
  venueId, 
  eventId, 
  title, 
  priceId, 
  ...
  const sharableLink = `https://venuapp.co.za/vendor-apply/${type}/${id}${priceParam}`;
  ...
  const getShareMessage = () => {
    ...
    return `Apply to become a vendor for ${title}${priceInfo}`;
  };
  ...
  const handleDownloadQR = () => {
    ...
    link.href = qrCodeUrl;
    link.download = `${title.replace(/\s+/g, '-')}-Vendor-QR.png`;
  ...
  const handleEmailShare = () => {
    const subject = encodeURIComponent(`Vendor Invitation for ${title}`);
    const body = encodeURIComponent(`Hello,\n\nYou're invited to apply as a vendor for ${title}.\n\nClick here to apply: ${sharableLink}\n\nThank you!`);
            ...
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex-shrink-0 flex justify-center items-start">
                <div className="bg-white p-4 rounded-lg shadow-sm border">
                  <img 
                    src={qrCodeUrl} 
                    alt={`QR code for vendor invitation`} 
              ...
              <div className="flex-grow space-y-4">
                ...
                <div className="mt-4">
                  ...
                  <ul className="text-sm text-gray-600 space-y-1 list-disc pl-5">
                    <li>Vendors scan this QR code or use the link to apply to your {type}</li>
                    <li>You'll receive a notification when a new application is submitted</li>
                    <li>Vendors will see the pricing and terms before applying</li>
                    <li>Upon approval, vendors can pay immediately through Venuapp</li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="md:w-1/2">
                <div className="mt-0 md:mt-0">
                  <h3 className="text-sm font-medium mb-2">How it works</h3>
                  <ul className="text-sm text-gray-600 space-y-1 list-disc pl-5">
                    <li>Share this link with vendors to apply to your {type}</li>
                    <li>You'll receive a notification when a new application is submitted</li>
                    <li>Vendors will see the pricing and terms before applying</li>
                    <li>Upon approval, vendors can pay immediately through Venuapp</li>
                    <li>You can track all applications in your vendor dashboard</li>
                  </ul>
                </div>
              </div>
            </div>
